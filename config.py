...
class Settings(BaseSettings):
    ...
    # Analysis-type runtime: sampling strategy used in the advanced EDA UI (core/eda/advanced_eda/granular_runtime/__init__.py)
    GRANULAR_RUNTIME_SAMPLING_STRATEGY: str = "random"
    # Analysis-type runtime: seed for deterministic sampling (core/eda/advanced_eda/granular_runtime/__init__.py)
    GRANULAR_RUNTIME_RANDOM_STATE: Optional[int] = 42
    # Analysis-type runtime: toggle for cached results in the analysis UI (core/eda/advanced_eda/granular_runtime/__init__.py)
    GRANULAR_RUNTIME_CACHE_ENABLED: bool = True
    # Analysis-type runtime: cache TTL for result reuse (core/eda/advanced_eda/granular_runtime/__init__.py)
    GRANULAR_RUNTIME_CACHE_TTL_SECONDS: int = 300
    # Analysis-type runtime: max cached entries for repeat runs (core/eda/advanced_eda/granular_runtime/__init__.py)
    GRANULAR_RUNTIME_CACHE_MAX_ITEMS: int = 128
    # Column insights / domain workflows: cache size backing async EDA service (core/eda/advanced_eda/services.py)
    GRANULAR_CACHE_MAX_SIZE: int = 256
    # Column insights UI: TTL for cached column insights payload (core/eda/advanced_eda/services.py)
    ...
    # The validator now treats None, 
    # empty strings, or strings like "none" / "unlimited" as “no limit.”
...
def get_settings() -> Settings:
    ...
    if env == "production":
        ...
        settings.init_production_features()
    elif env == "testing":
        settings = TestingSettings()
        settings.init_test_features()
    else:
        settings = DevelopmentSettings()
        settings.init_dev_features()
