...
Check the browser console for more detailed information.`);
    ...
    function moveToEDA() {
        ...
        if (!sourceId) {
            ...
            if (!sourceId) {
                showError('No dataset ID available for EDA navigation');
                return;
            }
        }
        ...
        const confirmed = confirm(
            `Move this dataset to Exploratory Data Analysis?\n\n` +
            `This will:\n` +
        ...
        if (window.DI && window.DI.utilities && window.DI.utilities.notifications) {
            window.DI.utilities.notifications.showLoading(true, 'Navigating to EDA...', 'Opening Exploratory Data Analysis interface...');
        } else {
            console.log('Navigating to EDA interface...');
        }
    ...
    function testEDANavigation() {
        console.log('Testing EDA navigation from data preview...');
        const testSourceId = 'test123';
        const edaUrl = `/advanced-eda/?source_id=${testSourceId}`;
        console.log('Test EDA URL:', edaUrl);
        if (confirm('Navigate to EDA test page from data preview?')) {
            window.location.href = edaUrl;
        }
    }
