    ...
    // Note: Preview state is managed through global.DI.state from globals module
        ...
        function closeDataPreviewModal() {
            ...
            global.DI.state.currentSourceId = null;
             ...
             // Move to EDA (Exploratory Data Analysis) function
        ...
        function moveToEDA() {
            ...
            // Show confirmation modal with EDA information
            const confirmed = confirm(
                `Move "${global.DI.state?.currentPreviewData?.name || 'Selected Dataset'}" to Exploratory Data Analysis?\n\n` +
                `This will:\n` +
            ...
            global.DI.utilities.notifications.showLoading(true, 'Navigating to EDA...', 'Opening Exploratory Data Analysis interface...');
            ...
            // Navigate to Advanced EDA module
            const edaUrl = `/advanced-eda/?source_id=${sourceId}`;
            console.log('Navigating to EDA URL:', edaUrl);
            window.location.href = edaUrl;
        }

        // Make moveToEDA globally accessible
        window.moveToEDA = moveToEDA;

        // Move to EDA from data source list (without opening modal)
        ...
        function moveToEDAFromList(sourceId, sourceName) {
            ...
            // Show confirmation modal with EDA information
            const confirmed = confirm(
                `Move "${sourceName || 'Selected Dataset'}" to Exploratory Data Analysis?\n\n` +
                `This will:\n` +
                `• Prepare the dataset for statistical analysis\n` +
                `• Generate data quality reports\n` +
                `• Create visualization dashboards\n` +
...
function startEDA(sourceId) {
    ...
    console.log('startEDA: Proceeding with navigation');
    global.DI.utilities.notifications.showLoading(true, 'Navigating to EDA...', 'Opening Exploratory Data Analysis interface...');
...
global.testEDANavigation = function() {
    console.log('Testing EDA navigation...');
    const testSourceId = 'test123';
    const edaUrl = `/advanced-eda/?source_id=${testSourceId}`;
    console.log('Test EDA URL:', edaUrl);
    if (confirm('Navigate to EDA test page?')) {
        window.location.href = edaUrl;
    }
};
